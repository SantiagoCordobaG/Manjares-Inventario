...
export function MobileNav({ profile }: { profile: Usuario }) {
  ...
  return <nav className="fixed bottom-0 left-0 right-0 z-50 grid grid-cols-4 border-t bg-white/95 p-2 backdrop-blur lg:hidden">{items.map((item) => <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 rounded-xl py-2 text-xs text-muted-foreground hover:bg-muted hover:text-foreground"><item.icon className="h-5 w-5" />{item.mobileLabel ?? item.label}</Link>)}</nav>;
}
