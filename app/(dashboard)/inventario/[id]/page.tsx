...
export default async function UnidadPage({ params }: { params: { id: string } }) {
  ...
  if (!unidad) notFound();
  const qrSrc = `/api/qr/${encodeURIComponent(unidad.qr_codigo)}`;
  return <div className="space-y-6"><div><h1 className="text-3xl font-semibold tracking-tight">{unidad.producto?.nombre}</h1><p className="font-mono text-sm text-muted-foreground">{unidad.qr_codigo}</p></div><div className="grid gap-4 lg:grid-cols-[360px_1fr]"><Card><CardHeader><CardTitle>QR y acciones</CardTitle></CardHeader><CardContent className="space-y-5"><div className="rounded-2xl border bg-white p-4 text-center"><Image src={qrSrc} alt={`QR de ${unidad.qr_codigo}`} width={220} height={220} unoptimized className="mx-auto h-[220px] w-[220px]" /><p className="mt-2 font-mono text-xs">{unidad.qr_codigo}</p></div><UnitActions unidadId={unidad.id} puntoActualId={unidad.punto_actual_id} puntos={puntos ?? []} /></CardContent></Card><div className="space-y-4"><Card><CardHeader><CardTitle>Detalle</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><p><span className="text-muted-foreground">Estado:</span> <Badge>{unidad.estado}</Badge></p><p><span className="text-muted-foreground">Punto:</span> {unidad.punto?.nombre ?? "Sin punto"}</p><p><span className="text-muted-foreground">Entrada:</span> {formatDate(unidad.fecha_entrada)}</p><p><span className="text-muted-foreground">Vencimiento:</span> {formatDate(unidad.fecha_vencimiento)}</p><p><span className="text-muted-foreground">Precio:</span> {formatCurrency(Number(unidad.producto?.precio ?? 0))}</p></CardContent></Card><Card><CardHeader><CardTitle>Historial</CardTitle></CardHeader><CardContent><Table><THead><TR><TH>Fecha</TH><TH>Tipo</TH><TH>Origen</TH><TH>Destino</TH><TH>Usuario</TH></TR></THead><TBody>{movimientos?.map((m: any) => <TR key={m.id}><TD>{formatDate(m.fecha)}</TD><TD>{m.tipo}</TD><TD>{m.origen?.nombre ?? "-"}</TD><TD>{m.destino?.nombre ?? "-"}</TD><TD>{m.usuario?.nombre ?? "Sistema"}</TD></TR>)}</TBody></Table></CardContent></Card></div></div></div>;
}
